...
(venv)~$ django-admin startproject ebdjango .  
...
The first step is to install and set up Postgresql on your machine, check the setup for PostrgreSQL for your setup here: PostgreSQL 
...
If you made it here, you have successfully installed django! 
...
Install ebhealthcheck (optiona) - If you are running an application load balanced environment, this will ensure that your health checks will work properly. Without it, you will run into health check issues in your environment.  

(venv)~$ pip install ebhealthcheck 
...
Now, you want to create a file within that .ebextensions directory and call it django.config  
...
Initialize the repository with eb init command: eb init 

Answer the question prompts to make sure you are setting it up in the correct region and with an SSH key (optional) 

Create an environment and deploy the application to it with eb create: eb create django-env 

This will go through a create the resources (may take a couple of minutes) 

Use eb status after everything is created to check the status: eb status 
...
Here are the settings.py updated settings: (You can find this info at realpython.com/deploying-a-django-app-to-aws-elastic-beanstalk/#configuring-a-database 

 

!!!Make sure to add the import.os above this or it will not work!  
...
Add a configuation file in .ebextensions called db-migrate.config (see instructions here https://docs.aws.amazon.com/elasticbeanstalk/latest/dg/create-deploy-python-django.html  
